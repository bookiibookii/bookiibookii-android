...
import com.bookiibookii.bookiibookii.databinding.FragmentDirectHostExtendPeriodDialogBinding
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.launch
import java.time.LocalDate
import java.time.format.DateTimeFormatter

class DirectHostExtendPeriodDialogFragment : DialogFragment() {

    private var _binding: FragmentDirectHostExtendPeriodDialogBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentDirectHostExtendPeriodDialogBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.etDays.doAfterTextChanged { text ->
            ...
            val color = if (hasInput) android.R.color.holo_orange_dark else R.color.grey_600
            binding.tvExtendedDate.setTextColor(ContextCompat.getColor(context, color))
        }
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                vm.event.collect { ev ->
                    when (ev) {
                        ...
                        is DirectHostEvent.ExtensionFail -> {
    ...
    companion object {
        const val TAG = "DirectExtendPeriodDialogFragment"
        private const val ARG_GROUP_ID = "arg_group_id"

        fun newInstance(groupId: Long) =
            DirectHostExtendPeriodDialogFragment().apply {
