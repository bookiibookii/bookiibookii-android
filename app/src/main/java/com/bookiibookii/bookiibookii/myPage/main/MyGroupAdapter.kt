...
import com.bookiibookii.bookiibookii.data.model.MypageGroup // DTO 확인
...
class MypGroupAdapter(private val items: List<MypageGroup>) :
    ...
    inner class ViewHolder(val binding: ItemMypGroupsBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: MypageGroup) {
            ...
            binding.itemMypGroupBookAuthor.text = item.auth
            ...
            item.groupTags.forEachIndexed { index, tag ->
            ...
            // 모집 상태 배지
            if (item.group_status == "RECRUITING") {
